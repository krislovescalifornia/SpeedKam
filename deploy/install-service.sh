...
echo
echo "NOTE: calibrate first so it reports speed -- drive past each way at a known"
echo "speed and set speed.d_east_m / speed.d_west_m in config.local.yaml from the"
echo "crossing time the log prints (the dashboard Calibration page has a"
echo "calculator). Confirm config.yaml points at the right camera. If you were"
echo "just added to the 'video' group, a reboot ensures it takes effect."
